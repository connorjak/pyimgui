...
# distutils: include_dirs = imgui-cpp imgui-terminal/include/terminal imgui-terminal/src/input
"""
Notes:
...
# https://stackoverflow.com/questions/3544240/conditional-ctypedef-with-cython
# cdef extern from "stdint.h":
#     ctypedef unsigned long long uint64_t
# from libc cimport stdint
# from stdint cimport uint16_t uint32_t uint64_t
# https://github.com/cython/cython/wiki/FAQ#how-do-i-declare-numeric-or-integer-c-types
from libc.stdint cimport uint32_t, uint64_t, uint16_t, uint8_t

cdef extern from "attribute.hh" namespace "terminal":
    ...
    #https://stackoverflow.com/questions/46962636/how-to-expose-static-constexpr-from-cython
    uint16_t ATTR_BOLD            = 0x0001;
    uint16_t ATTR_UNDERLINE       = 0x0002;
    uint16_t ATTR_BLINK           = 0x0004;
    uint16_t ATTR_REVERSE         = 0x0008;
    uint16_t ATTR_HIDDEN          = 0x0010;
    uint16_t ATTR_PROTECTED       = 0x0020; # See DECSCA, DECSEL, DECSED
    
    #! @todo these should be line attributes
    uint16_t ATTR_DBL_WIDE        = 0x0040;
    uint16_t ATTR_DBL_HIGH_TOP    = 0x0080;
    uint16_t ATTR_DBL_HIGH_BOTTOM = 0x0100;



cdef extern from "ascii_chars.hh" namespace "ascii":
    char NUL = 0x00;
    char SOH = 0x01;
    char STX = 0x02;
    char ETX = 0x03;
...
#include <cstdint>
#include <array>
# from libcpp cimport array


cdef extern from "colormap.hh" namespace "terminal":

    uint8_t NORMAL_BLACK   = 0;
    uint8_t NORMAL_RED     = 1;
    uint8_t NORMAL_GREEN   = 2;
    uint8_t NORMAL_YELLOW  = 3;
    uint8_t NORMAL_BLUE    = 4;
    uint8_t NORMAL_MAGENTA = 5;
    uint8_t NOMRAL_CYAN    = 6;
    uint8_t NORMAL_WHITE   = 7;

    uint8_t BRIGHT_BLACK   = 8;
    uint8_t BRIGHT_RED     = 9;
    uint8_t BRIGHT_GREEN   = 10;
    uint8_t BRIGHT_YELLOW  = 11;
    uint8_t BRIGHT_BLUE    = 12;
    uint8_t BRIGHT_MAGENTA = 13;
    uint8_t BRIGHT_CYAN    = 14;
    uint8_t BRIGHT_WHITE   = 15;

    #using colormap_t = std::array<uint32_t, 256>;
    # ctypedef array[uint32_t, 256] colormap_t 
    cdef extern from "<array>" namespace "std" nogil:
        cdef cppclass colormap_t "std::array<uint32_t, 256>":
            colormap_t() except+
            int& operator[](size_t)


#TODO rest of /include
